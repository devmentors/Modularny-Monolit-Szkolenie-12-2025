...
The \`AvailabilityModule\` class has been removed. Your task is to implement it.
...
Klasa \`AvailabilityModule\` została usunięta. Twoim zadaniem jest jej implementacja.
...
### Part 2: Replace Shared Contract with Local Contract in ParkingSpotsService

1. Replace \`IAvailabilityModuleApi\` with \`IModuleClient\` in constructor
...
{
    ResourceId = parkingSpot.Id,
    Capacity = ParkingSpotCapacity,
    Tags = new[] { "parking_spot" }
});
\`\`\`
...
### Część 2: Zamień Shared Contract na Local Contract w ParkingSpotsService

1. Zamień \`IAvailabilityModuleApi\` na \`IModuleClient\` w konstruktorze
...
- \`src/Modules/ParkingSpots/MySpot.Modules.ParkingSpots.Core/Services/ParkingSpotsService.cs\`
    `,
    hint: 'Start with AvailabilityModule - add the UseModuleRequests subscription first. Then modify ParkingSpotsService to use IModuleClient instead of IAvailabilityModuleApi. The anonymous object properties must match the AddResource command properties exactly.',
    hintPl: 'Zacznij od AvailabilityModule - najpierw dodaj subskrypcję UseModuleRequests. Następnie zmodyfikuj ParkingSpotsService, aby używał IModuleClient zamiast IAvailabilityModuleApi. Właściwości anonimowego obiektu muszą dokładnie odpowiadać właściwościom komendy AddResource.',
    solution: `
...
// ParkingSpotsService.cs - replace IAvailabilityModuleApi with IModuleClient:
...
- \`IModuleClient\` is the client-side interface for sending requests
...
- \`IModuleClient\` to interfejs po stronie klienta do wysyłania żądań
